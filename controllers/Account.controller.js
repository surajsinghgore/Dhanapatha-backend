...
export const addAccount = async (req, res) => {
  try {
    ...
    if (user.bankAccountDetails && user.bankAccountDetails.accountNumber === accountNumber && user.bankAccountDetails.ifscCode === ifscCode) {
...
export const updateAccount = async (req, res) => {
  try {
    ...
    if ((accountNumber && accountNumber !== user.bankAccountDetails.accountNumber) || (ifscCode && ifscCode !== user.bankAccountDetails.ifscCode)) {
      const existingUser = await User.findOne({
        "bankAccountDetails.accountNumber": accountNumber,
        "bankAccountDetails.ifscCode": ifscCode,
      });
...
export const fetchTransactions = async (req, res) => {
  try {
    ...
    const addMoneyTransactions = await AddMoney.find({ userId }).sort({ createdAt: -1 }).exec();
...
export const getTransactionsByReceiverEmail = async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({
      $or: [{ receiverId: receiver._id }, { senderId: receiver._id }],
    }).sort({ createdAt: 1 });
...
export const getSenderData = async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({ senderId: userId }).sort({ createdAt: -1 }).populate("receiverId", "username email").exec();
...
export const getReceiverData = async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({ receiverId: userId }).sort({ createdAt: -1 }).populate("senderId", "username email").exec();
